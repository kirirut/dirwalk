...
void process_entry(const char *dir_path, struct dirent *entry, int show_links, int show_dirs, int show_files, int sort) {
    char full_path[1024];
    snprintf(full_path, sizeof(full_path), "%s/%s", dir_path, entry->d_name);//Вывод полного пути
    ...
    int is_link = S_ISLNK(statusbuff.st_mode);//Ссылка?
    int is_dir = S_ISDIR(statusbuff.st_mode);//Каталог?
    ...
    if (is_dir) {
        walk_directory(full_path, show_links, show_dirs, show_files, sort);//Рекурсия 
    }
}